...
//  print_time("eu_id", queue, eu_id, NUM_WORK_GROUP);
  ...
  std::map<std::tuple<unsigned, unsigned>, std::vector<struct time_stamp>> time_map;
  ...
  for (unsigned i = 0; i < NUM_WORK_GROUP; ++i) {
    ...
    time_map[{ts.slice_id, ts.sub_slice_id}].push_back(ts);
  }
  ...
  for (unsigned i = 0; i < time_stamp.size(); i++) {
    ...
    if (i == 0) {
      printf("john lu work_group_id: %ld, slice_id:%ld, sub_slice_id: %ld, eu_id: %ld, entry: %ld, exit: %ld\n", ts.work_group_id, ts.slice_id, ts.sub_slice_id, ts.eu_id, ts.entry,
             ts.exit);
      continue;
    }
    auto& prev_ts = time_stamp[i-1];
    printf("john lu work_group_id: %ld, slice_id:%ld, sub_slice_id: %ld, eu_id: %ld, entry: %ld, exit: %ld, diff start: %ld\n", ts.work_group_id, ts.slice_id, ts.sub_slice_id, ts.eu_id, ts.entry,
           ts.exit, ts.entry - prev_ts.entry);
  }

  for (auto& kv : time_map) {
    printf("slice_id: %d, sub_slice_id: %d, total work group num: %zu\n", std::get<0>(kv.first), std::get<1>(kv.first), kv.second.size());
