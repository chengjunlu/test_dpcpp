...
ze_module_handle_t create_module(ze_device_handle_t device,
  ...
  auto ze_module_handle = create_module(device, filename, ZE_MODULE_FORMAT_IL_SPIRV, "", &buildlog);
  ...
  std::cout << "johnlu build log: " << strLog << std::endl;
  free(strLog);
  zeModuleBuildLogDestroy(buildlog);
  return (ze_module_handle);
}
